...
const TOOLKIT_ROUTES = ['/toolkit', '/segments', '/impact', '/diff-in-diff', '/timing', '/confidence-interval']

function App() {
  const location = useLocation()
  const toolkitActive = TOOLKIT_ROUTES.some(route => location.pathname.startsWith(route))
      ...
      <header className="top-header">
        ...
        <nav className="header-nav" aria-label="Main">
          <NavLink to="/" end className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
            <span className="nav-num">01</span>Plan
          </NavLink>
          <NavLink to="/analyze" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
            <span className="nav-num">02</span>Analyze
          </NavLink>
          <NavLink to="/diagnostics" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
            <span className="nav-num">03</span>Diagnose
          </NavLink>
          <NavLink to="/toolkit" className={`nav-item ${toolkitActive ? 'active' : ''}`}>
            Toolkit
          </NavLink>
        </nav>
